...
@pbot.on_message(filters.command("groupinfo", prefixes="/"))
async def get_group_status(_, message: Message):
    ...
    try:
        group = await pbot.get_chat(group_username)
    except Exception as e:
        await message.reply(f"Error: {e}")
        return
    
    total_members = await pbot.get_chat_members_count(group.id)
...
@pbot.on_message(filters.command("status") & filters.group)
...
__mod_name__ = "GʀᴏᴜᴘIɴғᴏ"
__help__ = """
 » /groupinfo : ʜᴇʟᴘs ᴛᴏ ғɪɴᴅ ʏᴏᴜʀ ɢʀᴏᴜᴘ ɪɴғᴏ.
 """
